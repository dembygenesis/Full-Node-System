...
const email = (function () {
    ...
    async function sendOverdueIncomingAlerts() {
    ...
    async function sendEmail(emailTo, content, subject) {
        try {
            ...
            /*const mailOptions = {
                from: "notifications@compliancelinc.com.au ",
                to: emailTo,
                subject: typeof subject === "undefined" ? "ComplianceLinc Account Created (no reply)" : subject,
    ...
    function sendItemNotificationEmail(emailTo, overdue, incoming) {
        ...
        sendEmail(emailTo, emailTemplate, "Items of Concern")
            .then(res => {
    ...
    async function sendActionInitiatedEmails(emails, complianceName, complianceId, actionRating) {

        let defectTemplate = '';

        if (actionRating === 'Critical Defect') {
            defectTemplate = 'critical-defect.html';
        }

        if (actionRating === 'Non-Critical Defect') {
            defectTemplate = 'non-critical-defect.html';
        }

        let template = await fetchTemplate(defectTemplate);
            template = template.replace('COMPLIANCE_NAME', complianceName);
            template = template.replace('COMPLIANCE_ID', complianceId);

        let subject = (actionRating === 'Critical Defect')
            ? 'CRITICAL DEFECT'
            : 'Non-Critical Defect';

        sendEmail(emails, template, subject);
    }

    async function sendManagementCompanyRegistrationEmail(email, password) {
        let template = await fetchTemplate('welcome-management-company.html');
            template = template.replace('USER_USERNAME', email);
            template = template.replace('USER_PASSWORD', password);

        sendEmail(email, template, 'Welcome To Compliancelinc');
    }

    async function sendManagementCompanyApproveEmail(email, password, userId) {
        let template = await fetchTemplate('approve-management-company.html');
        template = template.replace('USER_USERNAME', email);
        template = template.replace('USER_PASSWORD', password);
        template = template.replace('USER_ID', userId);

        sendEmail(email, template, 'Approve New User');
    }
    return {
        sendManagementCompanyRegistrationEmail: sendManagementCompanyRegistrationEmail,
        sendManagementCompanyApproveEmail: sendManagementCompanyApproveEmail,
